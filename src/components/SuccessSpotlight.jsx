    ...
    return (
      ...
      <div className="container mx-auto px-4 text-center">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          {stories.map(story => (
            <div key={story.id} className="card bg-base-100 shadow-xl border border-base-300 hover:scale-105 transition-transform duration-300 text-left">
              <div className="card-body">
                <div className="flex justify-between items-start mb-2">
                  <h3 className="card-title text-xl font-bold">{story.title}</h3>
                  <span className={`badge ${story.badgeColor} p-3 font-semibold`}>{story.tag}</span>
                </div>
                <p className="text-sm text-base-content/80 mb-4">{story.description}</p>
